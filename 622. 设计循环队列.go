/*
设计你的循环队列实现。 循环队列是一种线性数据结构，其操作表现基于 FIFO（先进先出）原则并且队尾被连接在队首之后以形成一个循环。它也被称为“环形缓冲器”。

循环队列的一个好处是我们可以利用这个队列之前用过的空间。在一个普通队列里，一旦一个队列满了，我们就不能插入下一个元素，即使在队列前面仍有空间。但是使用循环队列，我们能使用这些空间去存储新的值。

你的实现应该支持如下操作：

MyCircularQueue(k): 构造器，设置队列长度为 k 。
Front: 从队首获取元素。如果队列为空，返回 -1 。
Rear: 获取队尾元素。如果队列为空，返回 -1 。
enQueue(value): 向循环队列插入一个元素。如果成功插入则返回真。
deQueue(): 从循环队列中删除一个元素。如果成功删除则返回真。
isEmpty(): 检查循环队列是否为空。
isFull(): 检查循环队列是否已满。


示例：

MyCircularQueue circularQueue = new MyCircularQueue(3); // 设置长度为 3
circularQueue.enQueue(1); // 返回 true
circularQueue.enQueue(2); // 返回 true
circularQueue.enQueue(3); // 返回 true
circularQueue.enQueue(4); // 返回 false，队列已满
circularQueue.Rear(); // 返回 3
circularQueue.isFull(); // 返回 true
circularQueue.deQueue(); // 返回 true
circularQueue.enQueue(4); // 返回 true
circularQueue.Rear(); // 返回 4


提示：

所有的值都在 0 至 1000 的范围内；
操作数将在 1 至 1000 的范围内；
请不要使用内置的队列库。

作者：力扣 (LeetCode)
链接：https://leetcode-cn.com/leetbook/read/queue-stack/kzlb5/
来源：力扣（LeetCode）
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
*/

package main

import "fmt"

func main() {
	q := Constructor(3)
	fmt.Println(q.Front())   //-1
	fmt.Println(q.Rear())    //-1
	fmt.Println(q.IsEmpty()) //True

	fmt.Println(q.EnQueue(10)) //True
	fmt.Println(q.EnQueue(4))  //True
	fmt.Println(q.EnQueue(7))  //True
	fmt.Println(q.EnQueue(5))  //False
	fmt.Println(q.IsFull())    //True

	fmt.Println(q.Front()) //10
	fmt.Println(q.Rear())  //7

	fmt.Println(q.DeQueue()) //True
	fmt.Println(q.DeQueue()) //True
	fmt.Println(q.DeQueue()) //True
	fmt.Println(q.DeQueue()) //False
	fmt.Println(q.IsEmpty()) //True
}

type MyCircularQueue struct {
	head   int
	tail   int
	queue  []int
	length int
}

func Constructor(k int) MyCircularQueue {
	return MyCircularQueue{-1, -1, make([]int, k), k}
}

func (this *MyCircularQueue) EnQueue(value int) bool {
	if this.IsEmpty() {
		//首元素
		this.head++
		this.tail++
		this.to(this.tail, value)
		return true
	} else if this.IsFull() {
		return false
	}
	this.tail++
	this.to(this.tail, value)
	return true
}

func (this *MyCircularQueue) DeQueue() bool {
	if this.IsEmpty() {
		//已空
		return false
	}
	if this.head%this.length == this.tail%this.length {
		//最后一个元素
		this.head = -1
		this.tail = -1
		return true
	}
	this.head++
	return true
}

func (this *MyCircularQueue) Front() int {
	if this.IsEmpty() {
		return -1
	}
	return this.at(this.head)
}

func (this *MyCircularQueue) Rear() int {
	if this.IsEmpty() {
		return -1
	}
	return this.at(this.tail)
}

func (this *MyCircularQueue) IsEmpty() bool {
	return this.head == -1
}

func (this *MyCircularQueue) IsFull() bool {
	return this.head%this.length == (this.tail+1)%this.length
}

func (this *MyCircularQueue) at(i int) int {
	return this.queue[i%this.length]
}

func (this *MyCircularQueue) to(i int, value int) {
	this.queue[i%this.length] = value
}
